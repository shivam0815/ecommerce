// src/pages/Profile.tsx - FIXED VERSION
...
const getStatusColor = (status: string) => {
  switch (status.toLowerCase()) {
    case 'delivered': case 'paid': case 'cod_paid': return 'text-green-600 bg-green-100';
    case 'shipped': case 'processing': return 'text-blue-600 bg-blue-100';
    case 'pending': case 'awaiting_payment': case 'cod_pending': return 'text-yellow-600 bg-yellow-100';
    case 'cancelled': case 'failed': return 'text-red-600 bg-red-100';
...
const getStatusIcon = (status: string) => {
  switch (status.toLowerCase()) {
    case 'delivered': case 'paid': return <CheckCircleIcon className="w-4 h-4" />;
    case 'shipped': return <TruckIcon className="w-4 h-4" />;
    case 'processing': return <ClockIcon className="w-4 h-4" />;
    case 'cancelled': case 'failed': return <XCircleIcon className="w-4 h-4" />;
...
const Profile: React.FC = () => {
  ...
  const [orders, setOrders] = useState<Order[]>([]); // ✅ FIXED: Initialize as empty array
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      // ✅ FIXED: Ensure orders is always an array
      setOrders(Array.isArray(response.data.orders) ? response.data.orders : []);
      setError(null);
    } catch (err: any) {
      console.error('Failed to fetch orders:', err);
      setOrders([]); // ✅ FIXED: Set empty array on error
      if (err.response?.status !== 404) { // Don't show error for 404
  ...
  // ✅ Real-time Orders Polling (every 30 seconds)
  useEffect(() => {
    let intervalId: NodeJS.Timeout;

    if (activeTab === 'orders') {
      fetchOrders(); // Initial load
      intervalId = setInterval(fetchOrders, 30000); // Poll every 30s
    }
  ...
  // ✅ FIXED: Filtered Orders with proper array handling
  ...
  if (error && !user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-100 to-yellow-100">
        <motion.div
          className="w-[360px] bg-white/70 rounded-3xl shadow-2xl flex flex-col items-center p-10"
                ...
                >
                  <div className={clsx("flex items-center justify-center gap-2 mb-1", stat.color)}>
                    ...
                    <div className="text-2xl font-extrabold">
                      {stat.label === 'Total Spent' ? `₹${stats.totalSpent.toFixed(2)}` : stat.num}
                    </div>
                  </div>
        ...
        <div className="bg-white/80 backdrop-blur rounded-3xl shadow-xl">
          <div className="flex border-b">
            {[
              ...
              <button
                ...
                className={clsx("flex-1 py-4 px-6 text-center font-medium transition-all focus:outline-none text-lg", 
                  activeTab === tab.id
                    ? "text-green-600 border-b-4 border-green-400 bg-gradient-to-b from-green-100/60 to-white"
          ...
          <div className="p-8 min-h-[400px]">
            <AnimatePresence mode="wait">
              ...
              {/* Settings & Security tabs remain the same... */}
            </AnimatePresence>
          </div>
        </div>
      </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          type="button"
          className="flex-1 bg-white border border-gray-200 text-gray-700 py-3 px-6 rounded-xl font-semibold hover:bg-gray-100 transition active:scale-95 shadow"
        >
          📱 Change Password
        </button>
      </div>
      ...
      {!ordersLoading && orders.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6">
            {orderFilter === 'all' ? "You haven't placed any orders yet." : `No ${orderFilter} orders found.`}
          </p>
          <button 
            onClick={() => navigate('/')}
